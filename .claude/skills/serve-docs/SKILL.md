...
1. Check if Python is available (`python3 --version || python --version`)
   ...
   - If the user doesn't want to install Python, suggest `/serve-docs-docker` instead
...
- **pip not found**: Try `pip3` first, then suggest `/serve-docs-docker` as the zero-install alternative
