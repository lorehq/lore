...
if (LINKED) {
  const { rewriteForLinkedRepo } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'linked-rewrite'));
...
const { parseFrontmatter } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'frontmatter'));
const { TIER_ALIASES } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'generate-agents'));
const { getConfig: getAgentCfg } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'config'));
let agentTable = '';
try {
...
const { buildTree } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'tree'));
const { getConfig } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'config'));
...
const { getConfig: getCfg, getProfile: getProf } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'config'));
const { getAgentEntries: getAgents, getFieldnotes: getFieldnotesFn, getBannerLoadedSkills: getBLSkills } = require(path.join(SOURCE, '.lore', 'harness', 'lib', 'banner'));
