...
#
# Usage:
#   .lore/harness/scripts/lore-link.sh <target>          — link a work repo
#   .lore/harness/scripts/lore-link.sh --unlink <target> — remove link
#   .lore/harness/scripts/lore-link.sh --list            — show linked repos
#   .lore/harness/scripts/lore-link.sh --refresh         — regenerate all linked configs
...
do_link() {
  ...
  node -e "
    ...
    const h = (script, extra) => ({
      type: 'command',
      command: 'LORE_HUB=' + JSON.stringify(hub) + ' node ' + JSON.stringify(hub + '/.lore/harness/hooks/' + script) + (extra || '')
    });
    const settings = {
      hooks: {
  ...
  node -e "
    ...
    const config = {
      mcpServers: {
        'lore-search': {
          command: 'node',
          args: [hub + '/.lore/harness/mcp/search-server.js'],
          env: { LORE_HUB: hub }
        }
      }
    };
  ...
  node -e "
    const { rewriteForLinkedRepo } = require(process.argv[1] + '/.lore/harness/lib/linked-rewrite');
    const fs = require('fs');
    const hub = process.argv[1];
  ...
  bash "$HUB/.lore/harness/scripts/generate-cursor-rules.sh" --hub "$HUB" --target "$target" --linked "$HUB"
  ...
  node -e "
    ...
    const config = {
      ...
      mcp: {
        'lore-search': {
          type: 'local',
          command: ['node', hub + '/.lore/harness/mcp/search-server.js'],
          enabled: true,
...
case "${1:-}" in
  ...
  -h|--help)
    echo "Usage:"
    echo "  .lore/harness/scripts/lore-link.sh <target>          Link a work repo"
    echo "  .lore/harness/scripts/lore-link.sh --unlink <target> Remove link"
    echo "  .lore/harness/scripts/lore-link.sh --list            Show linked repos"
    echo "  .lore/harness/scripts/lore-link.sh --refresh         Regenerate all configs"
    ;;
