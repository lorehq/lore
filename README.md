...
**Semantic search and hot memory — highly recommended.** Tell your agent `/lore memory` to start the memory engine — semantic search over the knowledge base and Redis-backed hot memory for session context. Without Docker, agents fall back to Grep/Glob search and `memory.local.md`.
...
| `/lore memory burn` | Review hot cache facts, promote to persistent DATABANK |
| `/lore memory rem` | Knowledge defrag — restructure the DATABANK |
