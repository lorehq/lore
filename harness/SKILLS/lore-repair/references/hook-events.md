...
Lore exposes 7 canonical lifecycle events across 6 platforms. The binary dispatches to bundle/global/project scripts via `lore hook <event>`.

## Canonical Events

| Event | Command | Support | Purpose |
|-------|---------|---------|---------|
| PreToolUse | `lore hook pre-tool-use` | 6/6 | Allow, deny, or ask before tool execution |
| PostToolUse | `lore hook post-tool-use` | 6/6 | Inject context after tool execution |
| PromptSubmit | `lore hook prompt-submit` | 6/6 | Inject context or block before user message processing |
| SessionStart | `lore hook session-start` | 5/6 | Context injection at session init |
| Stop | `lore hook stop` | 6/6 | Agent response complete — log, archive, or block |
| PreCompact | `lore hook pre-compact` | 5/6 | Preserve state before context compression |
| SessionEnd | `lore hook session-end` | 4/6 | Session cleanup (best-effort) |
...
| Lore Event | Claude Code | Cursor | Copilot (Bot) | Gemini CLI | OpenCode | Windsurf |
|------------|------------|--------|---------------|------------|----------|----------|
| PreToolUse | `PreToolUse` | `preToolUse` | `preToolUse` | `BeforeTool` | `tool.execute.before` | `pre_write_code`, `pre_run_command`, `pre_read_code`, `pre_mcp_tool_use` |
| PostToolUse | `PostToolUse` | `postToolUse` | `postToolUse` | `AfterTool` | `tool.execute.after` | `post_write_code`, `post_run_command`, `post_read_code`, `post_mcp_tool_use` |
| PromptSubmit | `UserPromptSubmit` | `beforeSubmitPrompt` | `userPromptSubmitted` | `BeforeAgent` | `chat.message` | `pre_user_prompt` |
| SessionStart | `SessionStart` | `sessionStart` | `sessionStart` | `SessionStart` | `session.created` (event) | — |
...
| SessionEnd | `SessionEnd` | `sessionEnd` | `sessionEnd` | `SessionEnd` | `session.deleted` (event) | — |
