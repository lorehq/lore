...
CAPTURE: After substantive work \u2192 capture knowledge, create skills, validate consistency.`;
  ...
  const trees = [];
  const docsTree = buildTree(path.join(directory, 'docs'));
  if (docsTree.length > 0) trees.push('docs/\n' + docsTree.join('\n'));
  const skillsTree = buildTree(path.join(directory, '.lore', 'skills'));
  if (skillsTree.length > 0) trees.push('.lore/skills/\n' + skillsTree.join('\n'));
  const agentsTree = buildTree(path.join(directory, '.lore', 'agents'));
...
module.exports = { buildBanner, buildTree, ensureStickyFiles, getAgentDomains };
