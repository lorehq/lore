...
function buildTree(dir, prefix = '', options = {}) {
  ...
  } catch (e) { debug('buildTree: %s: %s', dir, e.message); }
  return lines;
}
...
function scanWork(dir) {
  ...
  } catch (e) { debug('scanWork: %s: %s', dir, e.message); }
  return active;
}
...
function getConfig(directory) {
  ...
  } catch (e) { debug('getConfig: %s', e.message); return {}; }
}
...
CAPTURE: After substantive work \u2192 capture knowledge, create skills, validate consistency.`;
  ...
  try {
    ...
    if (stripped) output += '\n\nPROJECT:\n' + stripped;
  } catch (e) { debug('agent-rules: %s', e.message); }
  ...
  } catch (e) { debug('conventions: %s', e.message); }
  ...
  } catch (e) { debug('local-memory: %s', e.message); }

  return output;
}

module.exports = { buildBanner, buildTree, ensureStickyFiles, getAgentDomains, getConfig, scanWork };
