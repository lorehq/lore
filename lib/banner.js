...
CAPTURE: After substantive work \u2192 capture knowledge, create skills, validate consistency.`;
  ...
  const skillsTree = buildTree(path.join(directory, '.claude', 'skills'));
  if (skillsTree.length > 0) trees.push('.claude/skills/\n' + skillsTree.join('\n'));
  const agentsTree = buildTree(path.join(directory, '.claude', 'agents'));
...
module.exports = { buildBanner, ensureStickyFiles, getAgentDomains };
