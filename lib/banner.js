...
function buildBanner(directory) {
  ...
  const delegationLine =
    "Orchestrate, don't execute \u2014 delegate to: " +
...
// Compact one-liner reminder for subsequent LLM calls after the first full banner.
// Modeled after hooks/prompt-preamble.js — bracketed parts joined with ' | '.
function buildCompactReminder(directory) {
  const agents = getAgentNames(directory);
  const parts = [];
  ...
  try {
    ...
    debug('buildCompactReminder conventions: %s', e.message);
  }
