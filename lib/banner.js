...
function buildBanner(directory) {
  ...
  const agentLine = agentEntries.length > 0
    ? agentEntries.map((a) => `${a.name} (${a.domain})`).join(' | ')
    : '(none yet)';
  const skillLine = operatorSkills.length > 0
    ? operatorSkills.map((s) => `${s.name} \u2014 ${s.description}`).join(' | ')
    : '';
