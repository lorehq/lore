...
function isKnowledgePath(filePath) {
  return filePath.includes('docs/') || filePath.includes('.lore/skills/') || filePath.includes('.claude/skills/');
}
...
function isDocsWrite(tool, filePath) {
  return isWriteTool(tool) && filePath.includes('/docs/');
}
...
function processToolUse({ tool, filePath, isFailure, bashCount, thresholds }) {
  ...
  if (newCount >= warn) {
    message = `>>> ${newCount} consecutive commands \u2014 capture what you learned \u2192 create-skill <<<`;
    level = 'warn';
  ...
  } else if (isWriteTool(tool) && filePath.includes('MEMORY.local.md')) {
    message = '>>> Gotcha buried in scratch notes? Move to /create-skill <<<';
  } else {
    message = 'Gotcha? \u2192 skill | New knowledge? \u2192 docs';
  }
...
function setNavDirty(flagPath) {
  if (flagPath && !fs.existsSync(flagPath)) {
    try { fs.writeFileSync(flagPath, Date.now().toString()); } catch {}
  }
}
