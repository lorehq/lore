...
test('getConfig: reads valid .lore-config', (t) => {
  ...
  fs.writeFileSync(path.join(dir, '.lore-config'), JSON.stringify({ version: '1.2.3', treeDepth: 4 }));
...
test('getConfig: returns empty object on malformed JSON', (t) => {
  const dir = setup();
  t.after(() => fs.rmSync(dir, { recursive: true, force: true }));
  fs.writeFileSync(path.join(dir, '.lore-config'), '{not valid json');
  const cfg = getConfig(dir);
  assert.deepStrictEqual(cfg, {});
});
